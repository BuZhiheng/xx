...
public class SplitOrderAdapter extends BaseQuickAdapter<SendBoxOrderDetailBean.ProductsBean, BaseViewHolder> {
    private LinkedHashMap<Integer, SendBoxOrderDetailBean.ProductsBean> map;
    private boolean isCustomInput;

    public SplitOrderAdapter(int layoutResId, List<SendBoxOrderDetailBean.ProductsBean> data) {
    ...
    protected void convert(final BaseViewHolder helper, SendBoxOrderDetailBean.ProductsBean item) {
        helper.setText(R.id.tv_box_type, item.getProduct_name() + " " + item.getProduct_color() + " " + item.getPlan_qty() + "个");
//        helper.setText(R.id.tv_box_color, item.getProduct_color());
//        helper.setText(R.id.tv_box_number, item.getPlan_qty() + "个");
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    if (isCustomInput) {// checkBOx没有选中，用户主动输入Edittext
                        mData.get(helper.getLayoutPosition()).setService_qty(Integer.valueOf(editText.getText().toString().trim()));
                        map.put(helper.getLayoutPosition(), mData.get(helper.getLayoutPosition())); // 记录选中的个数
                        isCustomInput = false;
                    } else {
                        map.put(helper.getLayoutPosition(), mData.get(helper.getLayoutPosition())); // 记录选中的个数
                        helper.setText(R.id.et_number, mData.get(helper.getLayoutPosition()).getPlan_qty() + ""); // 默认为planQty的个数
                    }
                } else {
        ...
        editText.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (TextUtils.isEmpty(s.toString())) { // 没有内容，取消选中
                    ...
                    // 判断输入的值不能大于总个数
                    if (Integer.valueOf(s.toString()) > mData.get(helper.getLayoutPosition()).getPlan_qty()) {
                        ToastUitl.showShort(mContext.getString(R.string.not_large) + mData.get(helper.getLayoutPosition()).getPlan_qty() + mContext.getString(R.string.individual));
                        editText.setText(mData.get(helper.getLayoutPosition()).getPlan_qty() + "");
                        ...
                        mData.get(helper.getLayoutPosition()).setService_qty(Integer.valueOf(s.toString()));
                        map.put(helper.getLayoutPosition(), mData.get(helper.getLayoutPosition())); // 记录选中的个数
                    }
                }
            }
