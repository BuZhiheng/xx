...
public class CarAdapter extends BaseQuickAdapter<CarNumberBean.DataBean, BaseViewHolder> {

    private boolean isDelete;

    private DeleteLinstener onDeleteListner;
    ...
    public CarAdapter(int layoutResId, List<CarNumberBean.DataBean> data) {
        super(layoutResId, data);
    }

    public void setOnDeleteListner(DeleteLinstener onDeleteListner) {
        this.onDeleteListner = onDeleteListner;
    }

    @Override
    protected void convert(final BaseViewHolder helper, final CarNumberBean.DataBean item) {
        ...
        helper.setText(R.id.tv_car_number, mContext.getString(R.string.order_inrtp_detail_plate) + item.getVehicle_id());
    ...
    public void deleteMulti() {
        ...
        for (int i = 0; i < mData.size(); i++) {
            if (mData.get(i).isSelect()) {
                deleteId.add(mData.get(i).getVehicle_id()); // 记录需要删除的司机id
    ...
    private void showDeleteDialog(final int layoutPosition) {
        ...
        dialog.isTitleShow(false)//
                .cornerRadius(5)//
                .content(mContext.getString(R.string.capacity_resourc_delete_driver))//
                ...
                new OnBtnClickL() {
                    ...
                    public void onBtnClick() {
                        ...
                        if (onDeleteListner != null)
                            onDeleteListner.onDeleteListner(mData.size() - 1, Arrays.asList(mData.get(layoutPosition).getVehicle_id()));
                        // 移除当前条目
